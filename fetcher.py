    ...
    try:
        ...
        for year in years:
            ...
            for day in reversed(list(range(day + 1))):
                d = retrieve_daily(daily_base_url, str(day),
                                   month_num, str(year))
                if d:
                    logger.info(
                    ...
                    str_day = "0" + str(day)
                    d = retrieve_daily(daily_base_url, str_day,
                                       month_num, str(year))
                    if d:
                        logger.info(
                            "Found {0} records for day {1}-{2}-{3}".format(len(d), day, month_num, year))
...
if __name__ == "__main__":
    ...
    # logger.info(result)
    # result = run_get_all(store_data=True, years=range(2017, 2021))
    result = run_get_all(True, range(2020, 2021), ['November'])
    # with open('data_dump.json', 'w') as fp:
